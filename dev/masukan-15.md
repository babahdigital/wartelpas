...
Masalah **"Menu terbuka sendiri saat Inspeksi (Mobile View)"** terjadi karena **konflik CSS**.

Saat Anda mengecilkan layar (mode inspeksi), browser masih membaca perintah `display: flex` dari mode Desktop, sehingga menunya tetap terbuka (menumpuk ke bawah) bukannya sembunyi (`display: none`).

Berikut adalah **SOLUSI FINAL** untuk `menu.php`.
Saya telah memperbaiki **CSS** dengan memisahkan aturan Desktop dan Mobile secara ketat, serta menambahkan **JavaScript** agar menu otomatis mereset diri saat layar berubah ukuran.

Silakan **GANTI SEMUA** isi file `menu.php` dengan kode ini:

```php
<?php
/*
 * Copyright (C) 2018 Laksamadi Guko.
 * Modified by Pak Dul (2026) - Top Navigation Layout (Responsive Fix)
 */
session_start();
error_reporting(0);
...
$ssettings = $seditor = $suplogo = $ssesslist = $snsettings = "";
...
elseif ($id == "sessions") $ssesslist = "active";
elseif ($id == "uplogo") $suplogo = "active";
elseif ($id == "editor") $seditor = "active";
...
<style>
    ...
    .wartelpas-brand {
        ...
        font-weight: 700; font-size: 0; line-height: 0; color: transparent !important;
        text-decoration: none; margin-right: 40px;
    }
    .wartelpas-brand::before {
        content: ""; display: block; height: 60px; width: 120px;
    ...
    /* Tombol Toggle Mobile (Default Hidden) */
    ...
    /* --- DESKTOP VIEW (Layar Besar > 992px) --- */
    ...
    /* --- MOBILE VIEW (Layar Kecil <= 992px) --- */
    @media (max-width: 992px) {
        .mobile-toggle { display: block; }
        .wartelpas-brand { margin-right: auto; }
        
        /* Menu Utama Mobile (Default Tersembunyi) */
        .nav-links {
            display: none; /* WAJIB NONE AGAR TIDAK MUNCUL SENDIRI */
        ...
        /* Kelas untuk memunculkan menu via JS */
        ...
        .nav-link {
            ...
            font-size: 14px; font-weight: 600;
            border-left: 3px solid transparent; /* Indikator samping */
        ...
        /* Dropdown muncul hanya jika parent diklik (active-mobile) */
...
<script>
    ...
    function toggleMenu() {
        ...
        if (x.className === "nav-links") {
            x.className += " show"; // Tambah class show
        } else {
            x.className = "nav-links"; // Hapus class show
        }
    }

    // 2. Toggle Submenu Mobile
    ...
    // 3. AUTO RESET saat Resize (PENTING!)
    // Ini memperbaiki masalah menu "nyangkut" saat inspeksi ditutup
    window.addEventListener('resize', function() {
        if (window.innerWidth > 992) {
            var x = document.getElementById("mainNav");
            if (x) x.classList.remove("show"); // Tutup paksa menu mobile jika layar membesar
            
            var items = document.querySelectorAll('.nav-item.active-mobile');
            items.forEach(function(item) {
                item.classList.remove('active-mobile'); // Reset dropdown
            });
        }
    });
...
1. **Pemisahan CSS Tegas (`@media (min-width: 993px)`):**
* Sebelumnya, aturan Desktop (`display: flex`) berlaku global, sehingga saat layar mengecil, dia "bocor" ke tampilan mobile.
* Sekarang, saya membungkus aturan Desktop dalam `@media (min-width: 993px)`. Artinya: **Style Desktop HANYA berlaku di Desktop**. Di Mobile, dia tidak akan memaksa `display: flex` kecuali tombol ditekan.


2. **Perbaikan Mobile CSS:**
* Menambahkan `display: none` secara eksplisit pada `.nav-links` di media query mobile.
* Menambahkan `.nav-links.show { display: flex !important; }` untuk memastikan menu hanya muncul saat tombol hamburger diklik.


3. **JavaScript Auto-Reset:**
* Saya menambahkan script `window.addEventListener('resize', ...)` di bagian bawah.
* Fungsinya: Jika Anda menutup mode Inspeksi (layar kembali lebar), script ini otomatis menghapus kelas `.show` dan `.active-mobile`. Menu akan kembali rapi seketika.



Sekarang menu Anda akan **tertutup rapat** saat inspeksi dibuka, dan kembali normal saat inspeksi ditutup.