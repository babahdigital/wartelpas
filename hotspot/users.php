        ...
        <td class="text-center">
          <?php if (in_array($req_status, ['all','used','rusak','online','retur'], true)): ?>
            ...
            <?php elseif (strtoupper($u['status']) === 'RETUR' && in_array($req_status, ['all','retur'], true)): ?>
              <button type="button" class="btn-act btn-act-print" onclick="window.open('./voucher/print.php?mode=status&status=retur&user=vc-<?= htmlspecialchars($u['name']) ?>&small=yes&session=<?= $session ?>','_blank').print()" title="Print Voucher Retur"><i class="fa fa-print"></i></button>
              <button type="button" class="btn-act btn-act-print" onclick="window.open('./voucher/print.php?mode=status&status=retur&user=vc-<?= htmlspecialchars($u['name']) ?>&small=yes&download=1&img=1&session=<?= $session ?>','_blank')" title="Download Voucher (PNG)"><i class="fa fa-download"></i></button>
...
?>
  ...
  <style>
    ...
    .confirm-btn i.fa.fa-print { position: relative; margin-right: 8px; }
