...
if ($is_usage && file_exists($dbFile)) {
    ...
    try {
        ...
        $db->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $stmt = $db->query("SELECT username, blok_name, ip_address, mac_address, last_uptime, last_bytes, login_time_real, logout_time_real, raw_comment, last_status, login_count, first_login_real, last_login_real, updated_at, validity FROM login_history");
    ...
    <table class="usage-table">
          ...
          <tbody>
              <?php if (empty($usage_list)): ?>
                  <tr><td colspan="<?= $is_ready_view ? 4 : 9 ?>" style="text-align:center;">Tidak ada data</td></tr>
              <?php else: ?>
