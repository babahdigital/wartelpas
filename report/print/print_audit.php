              ...
              FROM (
                SELECT
                  CASE
                    ...
                    ELSE 'normal'
                  END AS eff_status,
                  COALESCE(sh.price_snapshot, sh.price, sh.sprice_snapshot, 0) AS eff_price,
              ...
              FROM (
                SELECT
                  CASE
                    ...
                    ELSE 'normal'
                  END AS eff_status,
                  COALESCE(ls.price_snapshot, ls.price, ls.sprice_snapshot, 0) AS eff_price,
...
<body>
  <div class="toolbar">
    ...
    <?php if ($req_show === 'harian' && !empty($reuse_summary_by_block)): ?>
      <div style="border: 1px dashed #e0e0e0; background-color: #f8fafc; padding: 10px; margin-bottom: 16px; border-radius: 4px; font-size: 12px; color: #555;">
        <strong><i class="fa fa-refresh"></i> Catatan Reused (tidak dihitung penjualan):</strong><br>
        <div style="margin-top:6px;">
          <?php foreach ($reuse_summary_by_block as $bname => $items): ?>
            <?php
              ...
              if ($count <= 0) continue;
            ?>
            <div style="margin-top:2px;">Blok <?= htmlspecialchars($bname) ?>: <?= number_format($count,0,',','.') ?> voucher reused.</div>
          <?php endforeach; ?>
        </div>
      </div>
    <?php endif; ?>
      ...
      <?php
        ...
        $label_status = $selisih < 0 ? 'DEFISIT SETORAN' : ($selisih > 0 ? 'SURPLUS SETORAN' : 'SETORAN SEIMBANG');
      ?>
      ...
      <table style="width:100%; border:none;">
          <tr>
              ...
              <td style="border:none; padding:0; text-align:right;">
                  <div style="font-size:10px; color:#555; text-transform:uppercase;">Total Surplus / Defisit</div>
                  <div style="font-size:18px; font-weight:bold; color:<?= $text_color ?>; margin-top:4px;">
    ...
    <?php if (!empty($audit_manual_summary['total_refund'])): ?>
      <div style="margin-top:8px; padding:8px; border:1px solid #e5e7eb; background:#f8fafc; font-size:11px; color:#475569;">
        ...
        <?php if ($selisih != 0 && !empty($audit_refund_notes)): ?>
          <div style="margin-top:6px;">
            <strong>Catatan Surplus / Defisit (setelah refund/piutang):</strong>
