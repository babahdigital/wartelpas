...
try {
    if (file_exists($dbFile)) {
        ...
        $reuseExcludeLogin = table_exists($db, 'voucher_reuse_log')
            ? " AND NOT EXISTS (SELECT 1 FROM voucher_reuse_log vr WHERE vr.username = login_history.username AND substr(vr.reuse_date,1,7) = :d)"
            : '';
